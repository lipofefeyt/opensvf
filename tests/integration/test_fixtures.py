...
@pytest.mark.svf_fmus([FmuConfig(FMU_PATH, "counter")])
@pytest.mark.requirement("SVF-DEV-041")
def test_fixture_explicit_fmu_counter(svf_session) -> None:  # type: ignore[no-untyped-def]
    """Fixture starts SimpleCounter FMU. Counter should reach 1.0 within 2s default stop time."""
...
def test_fixture_no_fmu_raises(request) -> None:  # type: ignore[no-untyped-def]
    ...
    import inspect
    import svf.plugin.fixtures as fix_mod
    src = inspect.getsource(fix_mod)
    assert "svf_session requires at least one" in src
